#!/usr/bin/env bash
# "Claude remembers across sessions" — a self-contained demo of Ecphoria's cognition layer.
#
# It uses only curl against a running Ecphoria server, so it needs no API key. It shows:
...
#
# Usage:  ECPHORIA=http://localhost:8432 ./demo.sh
set -euo pipefail
ECPHORIA="${ECPHORIA:-http://localhost:8432}"
USER_ID="cust_42"
say() { printf '\n\033[1m== %s ==\033[0m\n' "$1"; }

say "Session 1 — Claude learns facts about $USER_ID"
curl -s -X POST "$ECPHORIA/api/v1/memories" -H 'Content-Type: application/json' \
...
SUP=$(curl -s -X POST "$ECPHORIA/api/v1/memories" -H 'Content-Type: application/json' \
...
curl -s -X POST "$ECPHORIA/api/v1/memories/search" -H 'Content-Type: application/json' \
...
curl -s "$ECPHORIA/api/v1/memories/$MEM_ID/history" | jq -c '.history[] | {state, content}'
