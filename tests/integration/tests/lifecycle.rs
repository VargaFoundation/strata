...
use ecphoria_gateway::server::{GatewayConfig, GatewayServer};

#[tokio::test]
async fn engine_starts_and_stops() {
    let engine = EcphoriaEngine::new(CoreConfig::default()).await.unwrap();
    engine.shutdown().await.unwrap();
}

#[tokio::test]
async fn full_stack_lifecycle() {
    let engine = Arc::new(EcphoriaEngine::new(CoreConfig::default()).await.unwrap());
