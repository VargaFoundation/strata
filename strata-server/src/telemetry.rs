...
//!
//! With the `otlp` feature built in AND `STRATA_OTLP_ENDPOINT` set, spans are exported to an OTLP
...
fn env_filter() -> EnvFilter {
    EnvFilter::try_from_default_env().unwrap_or_else(|_| "info,strata=debug".parse().unwrap())
}
...
pub fn init() -> TelemetryGuard {
    #[cfg(feature = "otlp")]
    {
        if let Ok(endpoint) = std::env::var("STRATA_OTLP_ENDPOINT") {
            if !endpoint.is_empty() {
                match build_otlp_provider(&endpoint) {
                    Ok(provider) => {
                        use opentelemetry::trace::TracerProvider as _;
                        let tracer = provider.tracer("strata");
