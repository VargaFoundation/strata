...
impl SnapshotManager {
    ...
    pub async fn build(engine: &Arc<StrataEngine>) -> crate::Result<Vec<u8>> {
        ...
        metrics::histogram!("strata_raft_snapshot_build_duration_seconds")
    ...
    pub async fn restore(engine: &Arc<StrataEngine>, data: &[u8]) -> crate::Result<()> {
        if data.is_empty() {
        ...
        // This replaces the in-memory index with the snapshot's contents.
        let vectors_dir = snap_dir.join("vectors");
        if vectors_dir.exists() {
            let semantic = engine.semantic_store();
            semantic
                .load_from(&vectors_dir)
                .map_err(|e| crate::Error::Replication(format!("semantic restore: {e}")))?;
        }
        ...
        let duration = start.elapsed();
        metrics::histogram!("strata_raft_snapshot_install_duration_seconds")
            .record(duration.as_secs_f64());
