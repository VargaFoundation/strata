//! Interactive SQL shell (REPL) for Strata.
...
pub async fn run(url: &str) -> anyhow::Result<()> {
    ...
    match client.health().await {
        Ok(h) => {
            ...
            println!("Connected to Strata ({url}), version {version}");
        }
        Err(e) => {
            anyhow::bail!("Cannot connect to Strata at {url}: {e}");
        }
    }
    ...
    loop {
        let readline = rl.readline("strata> ");
        match readline {
            Ok(line) => {
...
fn print_help() {
    println!("Type SQL queries to execute against the Strata server.");
    println!();
...
fn dirs_history_path() -> Option<std::path::PathBuf> {
    // Store history alongside other config in ~/.strata/
    let home = std::env::var("HOME").ok()?;
    let dir = std::path::PathBuf::from(home).join(".strata");
