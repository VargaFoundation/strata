...
| LLM proxy | **Working** | /v1/chat/completions with auto-RAG (semantic + episodic + **tenant-scoped user memories**), semantic response cache, multi-provider (OpenAI/Ollama/Anthropic), **single-turn tool-use** passthrough, **SSE streaming** (`stream:true`; Anthropic events translated to OpenAI chunks) |
