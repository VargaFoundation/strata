...
//!
//! Stands up a **real 3-node openraft cluster** — each node with its own `EcphoriaEngine` +
...
use ecphoria_cluster::raft::store::MemStore;
use ecphoria_cluster::raft::types::{AppRequest, NodeId, NodeInfo, TypeConfig};
...
fn raft_config() -> Arc<Config> {
    Arc::new(Config {
        cluster_name: "ecphoria-test".into(),
...
async fn three_node_cluster_replicates_and_converges() {
    ...
    let mut engines: BTreeMap<NodeId, Arc<EcphoriaEngine>> = BTreeMap::new();
    ...
    let ev = ecphoria_core::memory::episodic::Event::new("src", "e", serde_json::json!({"x": 1}));
    let ev_id = ev.id;
    rafts[&leader]
...
async fn cluster_forms_from_config_via_coordinator() {
    ...
    let ev = ecphoria_core::memory::episodic::Event::new("src", "e", serde_json::json!({"y": 2}));
    leader
        .client_write(AppRequest::Ingest {
            events: vec![ev],
            tenant: None,
...
async fn three_node_cluster_replicates_graph_supersede() {
    ...
    for id in 1..=3u64 {
        ...
        for _ in 0..100 {
            ...
            if edges.len() == 1
                && edges[0].id == edge_id
                && edges[0].state == ecphoria_core::memory::cognition::EdgeState::Superseded
