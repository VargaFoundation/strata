...
async fn three_node_grpc_cluster_replicates_over_sockets() {
    ...
    let ev = ecphoria_core::memory::episodic::Event::new("grpc", "e", serde_json::json!({"z": 9}));
    leader
...
/// Write one shared self-signed cert (SAN "ecphoria") + key, reused as the CA, and return a TLS config
/// pointing at them (mutual TLS — every node presents and trusts the same identity).
fn shared_tls(dir: &std::path::Path) -> ecphoria_cluster::config::RaftTlsConfig {
    let cert = rcgen::generate_simple_self_signed(vec!["ecphoria".to_string()]).unwrap();
    ...
    ecphoria_cluster::config::RaftTlsConfig {
...
async fn three_node_grpc_cluster_replicates_over_mtls() {
    ...
    let leader = coords
        ...
        .find(|c| c.is_leader())
        .expect("a leader coordinator");
    let ev = ecphoria_core::memory::episodic::Event::new("tls", "e", serde_json::json!({"t": 1}));
    leader
        .client_write(AppRequest::Ingest {
            events: vec![ev],
            tenant: None,
