...
use ecphoria_cluster::shard::ShardMove;
use ecphoria_cluster::{ClusterConfig, ClusterCoordinator, ShardedCluster};
...
async fn single_node(engine: Arc<EcphoriaEngine>) -> ClusterCoordinator {
...
async fn writes_route_to_owning_shard() {
    ...
    for k in &keys {
        ...
        let ev = ecphoria_core::memory::episodic::Event::new("s", "e", serde_json::json!({}));
        cluster
            .client_write(
                k,
                AppRequest::Ingest {
