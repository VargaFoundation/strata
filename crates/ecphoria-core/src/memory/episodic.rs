...
pub struct Event {
    ...
    // arity. See the round-trip regression test in ecphoria-cluster raft::types.
    #[serde(default)]
...
impl EpisodicStore {
    ...
    pub async fn append(&self, events: &[Event]) -> crate::Result<u64> {
        ...
        // Record metrics
        metrics::histogram!("ecphoria_episodic_append_duration_seconds")
            .record(start.elapsed().as_secs_f64());
        if let Ok(count) = &result {
            metrics::counter!("ecphoria_episodic_events_ingested_total").increment(*count);
        }
    ...
    pub fn query_sql_limited(
        ...
        metrics::histogram!("ecphoria_episodic_query_duration_seconds")
            .record(start.elapsed().as_secs_f64());
        metrics::counter!("ecphoria_episodic_queries_total").increment(1);
