...
impl QueryExecutor {
    ...
    ///
    /// For SQL queries containing `ecphoria_search()` or `ecphoria_state()` in
    ...
    pub async fn execute(
        ...
        match plan {
            QueryPlan::Sql(sql) => {
                // Try hybrid query rewriting for SQL containing ecphoria functions
    ...
    async fn execute_vector_search(
        ...
        let provider = self.embedding.as_ref().ok_or_else(|| {
            crate::Error::Embedding(
                "no embedding provider configured — ecphoria_search() requires an embedding provider"
                    .into(),
            )
        })?;
