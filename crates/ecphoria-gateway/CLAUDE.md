...
| gRPC | **Working** | tonic Query/Ingest/Search/State/Health **+ memory (add/search/get/delete/provenance) + sessions (start/end/recall) + graph (centrality/path/communities, tenant-scoped, optional as-of)** RPCs, 16MB max message. **Typed payloads** (`google.protobuf.Struct`/`Value` for rows/events/metadata/state values — not JSON-in-string). **Authenticated** (Bearer JWT in metadata) + **tenant-scoped** when auth_enabled |
