...
LLM proxy) into calls on `ecphoria_core::EcphoriaEngine`. Also handles authentication
...
| MCP Server | **Working** | **Streamable HTTP** at /mcp: POST (JSON-RPC; initialize returns `Mcp-Session-Id`) + GET (server→client SSE keep-alive) → native Claude Code **and** Claude Desktop. initialize, tools/list (24 tools), tools/call, resources/list, prompts/list. Authenticated when `auth_enabled`. See docs/connect-claude.md |
| MCP tools | **Working** | 24 tools: query (incl. `SELECT … FROM memories`), ingest, get_state, set_state, search, embed, start/end/recall_session + **memory** (add_memory, search_memory, get_memories [filters+offset], memory_history, delete_memory, update_memory, remember) + **graph** (link_memory, graph_neighbors) + **cognition/analytics** (memory_provenance, memory_feedback, list_contradictions, graph_centrality, graph_path, graph_communities) |
| gRPC | **Working** | tonic Query/Ingest/Search/State/Health **+ memory (add/search/get [filters+offset]/update/delete/provenance) + sessions (start/end/recall) + graph (centrality/path/communities, tenant-scoped, optional as-of)** RPCs, 16MB max message. **Typed payloads** (`google.protobuf.Struct`/`Value` for rows/events/metadata/state values — not JSON-in-string). **Authenticated** (Bearer JWT in metadata) + **tenant-scoped** when auth_enabled |
...
| POST | `/api/v1/attachments/search-image` | image-similarity search (body=query image; needs image embedder) | Yes* | **Working** |
...
- `cargo test -p ecphoria-gateway` (~157 tests, incl. gRPC tenant isolation + proxy tool-use translation + webhook signature schemes + SSRF guard)
