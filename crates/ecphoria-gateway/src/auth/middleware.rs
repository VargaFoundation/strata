...
pub struct AuthState {
    ...
    require_tenant: bool,
    /// Shared cluster secret used to authenticate the `x-ecphoria-shard-forwarded` marker, so a client
...
pub async fn require_auth(
    ...
    // double-count it. But only trust the `x-ecphoria-shard-forwarded` marker when it carries the
    ...
    let is_shard_forwarded = state.shard_forward_secret.as_ref().is_some_and(|secret| {
        req.headers()
            .get("x-ecphoria-shard-forwarded")
