...
/// Makes Ecphoria a drop-in for the embeddings API too (no auto-RAG here — this is pure embedding).
...
/// Ecphoria as a drop-in `base_url`.
pub async fn messages(
    State(engine): State<Arc<EcphoriaEngine>>,
    auth: Option<axum::Extension<crate::auth::middleware::AuthContext>>,
    Json(mut body): Json<serde_json::Value>,
) -> Response {
    let api_key = ecphoria_core::config::resolve_secret("ANTHROPIC_API_KEY");
    if api_key.is_empty() {
...
pub async fn chat_completions(
    ...
    if let Some(cached) = cached {
        metrics::counter!("ecphoria_llm_cache_hits_total").increment(1);
    ...
    metrics::counter!("ecphoria_llm_cache_misses_total").increment(1);
...
async fn stream_chat(
    http: &Client,
    config: &ecphoria_core::CoreConfig,
...
mod tests {
    ...
    fn retrieved_context_lands_in_the_user_turn_not_system() {
        ...
        assert!(last.contains("<ecphoria_retrieved_context>"));
        assert!(last.contains("</ecphoria_retrieved_context>"));
