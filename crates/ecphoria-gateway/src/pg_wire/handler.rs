//! PostgreSQL wire protocol handler — routes SQL to the Ecphoria engine.
...
/// password step (dev mode). Connect e.g. `psql "host=… user=ecphoria password=<API_KEY>"`.
...
#[async_trait]
impl StartupHandler for EcphoriaStartupHandler {
...
/// PG wire handler backed by the Ecphoria engine.
...
impl PgWireHandler {
    pub fn new(engine: Arc<EcphoriaEngine>, shard: Option<ShardRoutingState>) -> Self {
        Self {
            engine,
...
impl PgWireServerHandlers for PgWireFactory {
    type StartupHandler = EcphoriaStartupHandler;
...
mod tests {
    ...
    // Self-signed test cert/key (RSA-2048, CN=ecphoria-test) — TEST ONLY, never a real credential.
