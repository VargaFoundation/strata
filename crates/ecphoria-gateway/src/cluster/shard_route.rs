...
use axum::Json;
use ecphoria_cluster::ShardRouter;
use serde_json::json;

use crate::auth::middleware::AuthContext;

/// Header marking a request as already cross-shard-forwarded (loop guard).
const FORWARD_MARKER: &str = "x-ecphoria-shard-forwarded";
...
mod tests {
    ...
    async fn inmem_engine() -> std::sync::Arc<ecphoria_core::EcphoriaEngine> {
        ...
        std::sync::Arc::new(ecphoria_core::EcphoriaEngine::new(c).await.unwrap())
    }
    ...
    async fn rebalance_moves_tenant_across_pods() {
        ...
        let ta = ecphoria_core::config::TenantContext::new("t");
        engine_a
            .ingest_for_tenant(
                vec![ecphoria_core::memory::episodic::Event::new(
                    "s",
                    "e",
                    serde_json::json!({"x": 1}),
                )],
