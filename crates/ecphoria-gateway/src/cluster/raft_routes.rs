...
//! `ecphoria_cluster::raft::server::RaftGrpcServer` on the Raft port — see that module. These HTTP
...
use axum::Json;
use ecphoria_cluster::coordinator::EcphoriaRaft;

/// Shared state for cluster admin handlers.
#[derive(Clone)]
pub struct RaftState {
    pub raft: Arc<EcphoriaRaft>,
}

/// Request body for POST /cluster/add-learner.
#[derive(serde::Deserialize)]
pub struct AddLearnerRequest {
    pub node_id: ecphoria_cluster::raft::types::NodeId,
    pub addr: String,
}
...
pub async fn add_learner(
    ...
    let node_info = ecphoria_cluster::raft::types::NodeInfo { addr: req.addr };
    state
        .raft
...
pub struct ChangeMembershipRequest {
    pub members: std::collections::BTreeSet<ecphoria_cluster::raft::types::NodeId>,
}
...
/// gRPC, not here.
pub fn raft_router(raft: Arc<EcphoriaRaft>) -> axum::Router {
