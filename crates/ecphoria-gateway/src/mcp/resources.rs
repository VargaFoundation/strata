...
mod tests {
    ...
    fn list_resources_contains_episodic() {
        let resources = list_resources();
        assert!(resources.iter().any(|r| r.uri == "ecphoria://episodic"));
    }

    #[test]
    fn list_resources_contains_semantic() {
        let resources = list_resources();
        assert!(resources.iter().any(|r| r.uri == "ecphoria://semantic"));
    }

    #[test]
    fn list_resources_contains_state() {
        let resources = list_resources();
        assert!(resources.iter().any(|r| r.uri == "ecphoria://state"));
    }
    ...
    fn resource_serializes_to_json() {
        let resource = McpResource {
            uri: "ecphoria://test".into(),
            name: "Test".into(),
            description: "A test resource".into(),
        };
        let json = serde_json::to_value(&resource).unwrap();
        assert_eq!(json["uri"], "ecphoria://test");
    }
}
