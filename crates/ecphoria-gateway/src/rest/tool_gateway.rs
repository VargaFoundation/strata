//! MCP tool-gateway — a governed registry of **downstream** MCP servers that Ecphoria can call.
//!
//! Today Ecphoria's MCP server *exposes* its own data tools; this is the other half: registering
...
/// Bridge so the agent loop (in `ecphoria-core`) can invoke downstream MCP tools through the gateway.
#[async_trait::async_trait]
impl ecphoria_core::runtime::ToolExecutor for ToolGateway {
    async fn call_tool(
        &self,
        server: &str,
        tool: &str,
        arguments: serde_json::Value,
    ) -> ecphoria_core::Result<serde_json::Value> {
        self.call(server, tool, arguments)
            .await
            .map_err(ecphoria_core::Error::Ingest)
    }
}
