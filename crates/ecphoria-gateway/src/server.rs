...
pub struct GatewayConfig {
    ...
    /// Off by default — an unauthenticated Ecphoria is an open read/write database.
    #[serde(default)]
    ...
    /// Accepts a TOML array or a comma-separated string (`ECPHORIA_GATEWAY__API_KEYS="k1,k2"`).
                ...
                if config.allow_insecure {
                    ...
                    return Err(crate::Error::Auth(format!(
                        "refusing to start: auth_enabled=false while binding non-loopback \
                         ...
                         auth_enabled=true + api_keys (ECPHORIA_GATEWAY__AUTH_ENABLED=true, \
                         ECPHORIA_GATEWAY__API_KEYS=\"<key>\"); (2) bind loopback only, e.g. \
...
mod tests {
    ...
    fn api_keys_deserialize_from_comma_string() {
        // The env-var shape: ECPHORIA_GATEWAY__API_KEYS="k1@acme,k2@beta:reader".
        let config: GatewayConfig =
