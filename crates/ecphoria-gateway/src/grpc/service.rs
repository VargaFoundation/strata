//! gRPC service implementation backed by EcphoriaEngine.
...
use crate::grpc::proto::ecphoria_server::Ecphoria;

/// gRPC service implementation.
pub struct EcphoriaGrpcService {
...
#[tonic::async_trait]
impl Ecphoria for EcphoriaGrpcService {
    async fn query(
        &self,
    ...
    async fn ingest(
        ...
        let result = match tenant {
            Some(t) => {
                self.engine
                    .ingest_for_tenant(events, &ecphoria_core::config::TenantContext::new(t))
                    .await
            }
...
pub async fn start_grpc(
    ...
    let service = EcphoriaGrpcService::new(engine, auth, shard);
    ...
    tokio::spawn(async move {
        if let Err(e) = tonic::transport::Server::builder()
            .add_service(
                proto::ecphoria_server::EcphoriaServer::new(service)
...
mod tests {
    ...
    async fn svc() -> EcphoriaGrpcService {
        ...
        EcphoriaGrpcService::new(engine, Some(auth), None)
    }
    ...
    async fn svc_sharded(my_shard: usize) -> EcphoriaGrpcService {
        let mut config = ecphoria_core::CoreConfig::default();
        config.memory.episodic.db_path = ":memory:".into();
        config.memory.state.db_path = ":memory:".into();
        config.memory.cognition.db_path = ":memory:".into();
        ...
        let shard = crate::cluster::shard_route::ShardRoutingState {
            router: std::sync::Arc::new(ecphoria_cluster::ShardRouter::new(2, 128)),
            my_shard,
            base_urls: std::sync::Arc::new(vec!["http://s0".into(), "http://s1".into()]),
            http: reqwest::Client::new(),
            forward_secret: None,
        };
        EcphoriaGrpcService::new(engine, Some(auth), Some(shard))
    }

    fn tenant_on_shard(shard: usize) -> String {
        let router = ecphoria_cluster::ShardRouter::new(2, 128);
        (0..)
            .map(|i| format!("t{i}"))
