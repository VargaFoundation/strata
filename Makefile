# Ecphoria developer Makefile. The build targets run the disk guard FIRST so a runaway target/
...
cluster-sharded:  ; bash ops/cluster-local/run-sharded.sh && bash ops/cluster-local/sharding-test.sh; RUN_DIR=/tmp/ecphoria-sharded bash ops/cluster-local/stop-cluster.sh
